...
pub trait Decoder {
    ...
    fn decode(
        &mut self,
        buffer: Buffer,
    ) -> Result<ImageBuffer<Self::OutputPixels, Self::PixelContainer>, Self::Error>;
    ...
    fn decode_buffer(
        &mut self,
        buffer: &mut [<<Self as Decoder>::OutputPixels as Pixel>::Subpixel],
    ) -> Result<(), Self::Error>;
...
pub trait AsyncDecoder: Decoder {
    ...
    async fn decode_buffer(
        &mut self,
        buffer: &mut [Self::OutputPixels::Subpixel],
    ) -> Result<(), Self::Error>;
}
...
pub trait AsyncIdemptDecoder: Decoder {
    /// Asynchronous decoder
    async fn decode_nm_async(
        &self,
        buffer: Buffer,
    ) -> Result<ImageBuffer<Self::OutputPixels, Self::PixelContainer>, Self::Error>;

    /// Asynchronous decoder to user buffer.
    async fn decode_nm_buffer(
        &self,
        buffer: &mut [Self::OutputPixels::Subpixel],
    ) -> Result<(), Self::Error>;
}
